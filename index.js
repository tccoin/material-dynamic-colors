...
exports.materialDynamicColors = exports.intFromRgb = exports.index_seedFromImage = void 0;
var BASELINE_1P = {
  light: {
    background: '#FFFFFF',
    surface: '#FFFFFF'
...
const TonalPalette = class {
  ...
  tone(tone) {
    ...
    void 0 === argb && (argb = new HCT(this.hue, this.chroma, tone).toInt(), this.cache.set(tone, argb));
    return argb;
  }
...
const score = function (colorsToPopulation) {
  ...
  for (const [color_tsickle_destructured_3, cam_tsickle_destructured_4] of colorsToCam.entries()) {
    ...
    for (let i = hue - 15; i < hue + 15; i++) excitedProportion += hueProportions[math_utils_sanitizeDegrees(i)];
...
const createBoxes = function (self) {
  ...
  for (let i = 1; 256 > i; i++) {
    cut(self, self.cubes[next], self.cubes[i]) ? (volumeVariance[next] = 1 < self.cubes[next].vol ? variance(self, self.cubes[next]) : 0, volumeVariance[i] = 1 < self.cubes[i].vol ? variance(self, self.cubes[i]) : 0) : (volumeVariance[next] = 0, i--);
    next = 0;
...
const index_unwrapSafeUrl = function (url) {
  if (url instanceof SafeUrl) {
    if (url instanceof safe_url_impl_SafeUrlImpl) var _temp = url.privateDoNotAccessOrElseWrappedUrl;else throw Error('Unexpected type when unwrapping SafeUrl');
...
const safe_url_builders_fromBlob = function (blob) {
  ...
  if (2 !== (null === match || void 0 === match ? void 0 : match.length) || !(/^image\/(?:bmp|gif|jpeg|jpg|png|tiff|webp|x-icon)$/i.test(match[1]) || /^video\/(?:mpeg|mp4|ogg|webm|x-matroska|quicktime|x-ms-wmv)$/i.test(match[1]) || /^audio\/(?:3gpp2|3gpp|aac|L16|midi|mp3|mp4|mpeg|oga|ogg|opus|x-m4a|x-matroska|x-wav|wav|webm)$/i.test(match[1]))) throw Error(`unsafe blob MIME type: ${blob.type}`);
...
const image_utils_bufferToPixels = async function (buffer) {
  ...
  for (let i = 0; i < imageData.data.length; i += 4) 255 > imageData.data[i + 3] || pixels.push(intFromRgb([imageData.data[i], imageData.data[i + 1], imageData.data[i + 2]]));

  return pixels;
};

const index_seedFromImage = async function (image, pixels = null) {
  if (!pixels) {
    const imageBuffer = 'string' === typeof image ? await (await fetch(image)).arrayBuffer() : image;
    pixels = await image_utils_bufferToPixels(imageBuffer);
  }

  var self = new QuantizerWu(),
      a;
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    void 0 === pixelCount ? (pointCount++, points.push(labFromInt(inputPixel)), pixels_.push(inputPixel), pixelToCount.set(inputPixel, 1)) : pixelToCount.set(inputPixel, pixelCount + 1);
  }
  ...
  if (0 === colors.length && 0 < additionalClustersNeeded) for (let i = 0; i < additionalClustersNeeded; i++) clusters.push([100 * Math.random(), 201 * Math.random() + -100, 201 * Math.random() + -100]);
  ...
  for (let i = 0; i < clusterCount; i++) {
    ...
    for (let j = 0; j < clusterCount; j++) distanceToIndexMatrix[i].push(new quantizer_wsmeans_DistanceAndIndex());
  }
...
exports.index_seedFromImage = index_seedFromImage;
...
const materialDynamicColors = async function (from) {
  ...
  if (from.target && from.target.files[0]) image = URL.createObjectURL(from.target.files[0]);
